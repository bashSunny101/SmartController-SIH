...
async function initDb() {
  ...
  // Seed demo alerts, honeypot and hardware data if empty (makes frontend show realistic data)
  const alertsCount = (await db.get('SELECT COUNT(*) as c FROM alerts')).c;
  if (!alertsCount) {
    const now = new Date().toISOString();
    await db.run('INSERT INTO alerts(alert_id, type, severity, attacker_ip, action, createdAt) VALUES(?,?,?,?,?,?)', 'ALERT-001', 'intrusion', 'high', '203.0.113.5', 'blocked', now);
    await db.run('INSERT INTO alerts(alert_id, type, severity, attacker_ip, action, createdAt) VALUES(?,?,?,?,?,?)', 'ALERT-002', 'malware', 'medium', '198.51.100.23', 'quarantined', now);
  }
  const hpCount = (await db.get('SELECT COUNT(*) as c FROM honeypot')).c;
  if (!hpCount) {
    const now = new Date().toISOString();
    await db.run('INSERT INTO honeypot(alert_id, type, severity, attacker_ip, action, createdAt) VALUES(?,?,?,?,?,?)', 'HP-001', 'ssh-bruteforce', 'high', '203.0.113.7', 'logged', now);
    await db.run('INSERT INTO honeypot(alert_id, type, severity, attacker_ip, action, createdAt) VALUES(?,?,?,?,?,?)', 'HP-002', 'http-probe', 'low', '198.51.100.45', 'logged', now);
  }
  const hwCount = (await db.get('SELECT COUNT(*) as c FROM hardware')).c;
  if (!hwCount) {
    await db.run('INSERT INTO hardware(hw_id, name, type, patched) VALUES(?,?,?,?)', 'HW-001', 'Edge Router 1', 'router', 0);
    await db.run('INSERT INTO hardware(hw_id, name, type, patched) VALUES(?,?,?,?)', 'HW-002', 'Battery Bank A', 'battery', 1);
  }
}
...
app.post('/user/verify_otp', async (req, res) => {
  ...
  // allow demo bypass via env during testing
  ...
  // validate OTP
  const row = await db.get('SELECT * FROM otp_codes WHERE empID = ? ORDER BY id DESC LIMIT 1', empID);
  if (!row) return res.status(400).json({ error: 'OTP not requested' });
  const now = Date.now();
  if (row.expiresAt < now) return res.status(400).json({ error: 'OTP expired' });
  const valid = await bcrypt.compare(String(otp), row.code_hash);
...
// Request OTP endpoint: generates OTP, stores hashed code and optionally emails it
app.post('/user/request_otp', async (req, res) => {
  ...
  await db.run('INSERT INTO otp_codes(empID, code_hash, expiresAt) VALUES(?,?,?)', empID, hash, expiresAt);
  ...
  if (process.env.SMTP_HOST && process.env.SMTP_USER) {
    try {
      ...
      await transporter.sendMail({
        ...
        text: `Your OTP code is ${otp} (valid 5 minutes)`
      });
    } catch (err) {
      console.error('Failed to send OTP email', err);
    }
  }
  // return OTP in response if demo mode enabled (for tests/dev)
  if (process.env.ALLOW_DEMO_OTP === '1') {
    return res.json({ demoOtp: otp });
  }
